        ...
        <div className="h-full flex items-center justify-center lg:justify-end">
          <div className="md:ml-10 flex items-baseline space-x-4">
            {navLinks.map(({ path, name, active, hash }, index) => {
              if (hash) {
                return (
                  <HashLink key={path} onMouseOver={() => onHoverLink(index)} onMouseOut={() => onHoverLink(index)} to={path} className="relative leading-relaxed capitalize text-white  hover:text-gray-400 px-3 py-2 text-md font-light">
                    <p>{name}</p>
              ...
              return (
                <Link key={path} onMouseOver={() => onHoverLink(index)} onMouseOut={() => onHoverLink(index)} to={path} className="relative leading-relaxed capitalize text-white  hover:text-gray-400 px-3 py-2 text-md font-light">
                  <p>{name}</p>
                  {location.pathname.split('/')[1] === path.substring(1) && <div style={{ bottom: -17, backgroundColor: active ? '#cbd5e0' : 'white' }} className="h-1 w-full relative"></div>}
                </Link>
              )
            })}
          </div>
