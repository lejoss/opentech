	...
	return (
		<figure className="flex flex-col sm:mr-2 mb-2 py-4 bg-white sm:w-5/12 lg:w-64 shadow-md rounded-md">
			...
			{person.projects && person.projects.length > 0 && (
				<Link to={`/projects?person=${person.id}`} className="rounded-full border border-indigo-600 hover:bg-indigo-100 flex justify-center p-2 mx-8 shadow-sm cursor-pointer mt-6">
