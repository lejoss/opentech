...
function ProjectCard({ project, people = null }) {
	return (
		<figure className="flex flex-col sm:mr-2 mb-2 py-6 px-4 bg-white sm:w-5/12 lg:w-64 shadow-sm rounded-md">
			<img className="sm:w-48 my-0 mx-auto h-32" src={project.img || "/images/broken_image.svg"} alt="tpp" />
			...
			<Link to={`/projects/${project.id}/intro`} className="rounded-full border border-indigo-700 hover:bg-indigo-100 flex justify-center p-2 mx-4 shadow-sm cursor-pointer mt-6">
